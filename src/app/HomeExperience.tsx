...
export function HomeExperience({ isAuthenticated }: { isAuthenticated: boolean }) {
  ...
  // Registration is required before stepping inside — it's how mentors and
  // administrators get an applicant profile to track, and how a completed
  // pathway's certificate finds its way back to the right person.
  function enterHallway() {
    if (!isAuthenticated) {
      router.push(`/register?callbackUrl=${encodeURIComponent("/?entered=1")}`);
      return;
    }
    setWelcomed(true);
  }
